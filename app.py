...
st.markdown(f"""
    <style>
        ...
        .stButton button {{
            ...
            color: white !important; border-radius: 20px !important; border: 1px solid {current_accent_color} !important;
            font-weight: bold !important; box-shadow: 0 0 15px {current_accent_color}66;
        }}
    </style>
...
# --- ЭТАП 1: ТЕКСТОВЫЙ ВВОД ДАТЫ (Идеально для любых годов) ---
if st.session_state.stage == 1:
    st.markdown("### 🪐 Шаг I: Точка входа в матрицу")
    with st.form("stage1_form"):
        # Изменили на ручной ввод, чтобы легко писать любой год (хоть 1940, хоть 1976)
        ...
        if st.form_submit_button("🔑 Рассчитать Код Судьбы"):
            if not user_contact:
                ...
                try:
                    ...
                    user_date = datetime.strptime(user_date_str.strip(), "%d.%m.%Y").date()
                    
                    # Проверяем адекватность года на всякий случай
