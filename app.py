...
if st.session_state.stage == 1:
    ...
    with st.form("stage1_form"):
        ...
        if st.form_submit_button("🔑 Рассчитать Код Судьбы"):
            ...
            elif not user_date_str: st.error("Пожалуйста, укажите дату рождения.")
            else:
                try:
                    ...
                    if user_date.year < 1920 or user_date.year > date.today().year: st.error("Укажите год от 1920.")
                    else:
                        ...
                        st.rerun()
                except ValueError: st.error("❌ Неверный формат! Введите дату через точки. Пример: 24.10.1976")
