...
import requests
import urllib.parse
from content import CORE_DATA # Прямой безопасный импорт текстов
...
if st.session_state.stage == 1:
    ...
    if st.button("🔑 Рассчитать Код Судьбы", key="btn_submit1"):
        if not user_contact or not user_date_str: st.session_state.error_text = "Заполните все поля."
        else:
            try:
                ...
                st.rerun()
            except: st.session_state.error_text = "❌ Неверный формат даты! Пример: 05.08.1997"

    if st.session_state.error_text and st.session_state.num_code is None: st.error(st.session_state.error_text)

if st.session_state.stage == 2:
    profile = CORE_DATA.get(st.session_state.num_code, CORE_DATA["1"])
    st.header(f"✨ {profile['title']}")
    
    st.markdown("#### 🧠 1. Психология и ментальные блоки:")
    st.info(profile['p'])
    st.markdown("#### 💼 2. Бизнес, Финансы и Реализация:")
    st.info(profile['f'])
    st.markdown("#### ❤️ 3. Отношения и Личные границы:")
    st.info(profile['o'])
    st.markdown("#### 🚀 4. Главный шаг для прорыва застоя:")
    st.success(profile['a'])
    
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown(f"### 🧪 Шаг II: Глубокое сканирование сознания")
    ...
    if st.button("📊 Скомпилировать финальный отчет", key="submit_stage2"):
        ...
        st.markdown("#### 📢 Поделиться инсайтом с друзьями:")
        share_text = f"Прошел Digital Oracle. Мой вердикт застоя: {final_report} \n\nРазблокируй свой код судьбы на сайте: https://streamlit.app"
