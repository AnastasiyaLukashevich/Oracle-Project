...
def get_deep_test_for_number(num_key):
    ...
    if num_key in tests: return tests[num_key]
...
def calculate_dynamic_test_result(num_key, selected_answers):
    ...
    if total_score <= 2: return test_data["results"]["low"]
    elif total_score <= 4: return test_data["results"]["medium"]
    else: return test_data["results"]["high"]
